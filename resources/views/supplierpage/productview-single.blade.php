@extends('layout.app')
@section('title',"$sellerproduct->product_name")
@push('headerscript')
@endpush
        ...
        <div class="row">
            <div class="col-md-offset-2 col-md-8">
                <div class="page-header text-center">
                    <h1>{{ $sellerproduct->product_name }}</h1>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="row">
                        <div class="col-md-4 no-right-pd">
                            <div class="vendor-image">
                                <!-- venue pic -->
                                <a href="#"><img src="{{ asset($sellerproduct->featured_image) }}" alt="wedding venue" class="img-responsive"></a>
                            </div>
                        </div>
                        <!-- /.venue pic -->
                        <div class=" col-md-8 no-left-pd">
                            ...
                            <div class="vendor-list-details">
                                <div class="caption">
                                    <!-- caption -->
                                    <h2>{{ $sellerproduct->product_name }}</h2>
                                    <p>(Want to {{ ($sellerproduct->post_type==1)?"Buy":"Sell" }})</p>
                                    <p class="location"><i class="fa fa-map-marker"></i> {{ $sellerproduct->seller->state.", ".$sellerproduct->seller->sellercountry->country_name }}</p>
                                    <div class="rating"> <i class="fa fa-star"></i> <i class="fa fa-star"></i> <i class="fa fa-star"></i> <i class="fa fa-star"></i> <i class="fa fa-star"></i></div>
                                </div>
                                <!-- /.caption -->
                                <div class="vendor-price">
                                    <div class="price">{{ ($sellerproduct->product_price)?"$".$sellerproduct->product_price:'' }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="st-tabs">
                    ...
                    <div class="tab-content">
                        ...
                        <div role="tabpanel" class="tab-pane fade active in" id="myListing">
                            {!! $sellerproduct->full_description !!}
                        </div>
                        <div role="tabpanel" class="tab-pane fade" id="about">
                            {!! $sellerproduct->seller->sellerprofile->profile_description !!}
                        </div>
                        <div role="tabpanel" class="tab-pane fade" id="makeEnquiry">
                            <div class="venue-details">
                                ...
                                @endif
                                <form id="makeenq" method="post"  action="{{ url("enquiry-for-seller/".$sellerproduct->seller->id) }}">
                                    @csrf
                                    <!-- Text input-->
                    ...
                    <div id="productcaro" class="owl-carousel owl-theme mycsutom">
                        @foreach($sellerproduct->seller->product as  $pro)
