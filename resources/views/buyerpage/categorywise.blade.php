...
@endphp
@section('title',"Top $catrow->category_name Buyer's $search[for] in $mytype")
@section('content')
        ...
        <div class="row">
            <div class="col-md-offset-2 col-md-8">
                <div class="page-header text-center">
                    <h1>Top {{ $catrow->category_name }} Buyer's {{ $search['for'] }} in $mytype</h1>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="vendor-list-details">
                                <div class="caption">
                                    <!-- caption -->
                                    <h2><a href="{{ url("buyer/$row->company_slug/$row->unique_code") }}" class="title">{{ $row->first_name }}</a></h2>
                                    <p class="location"><i class="fa fa-map-marker"></i> {{ "$row->state, " }} {{ CommonClass::GetCountryName($row->country) }}</p>
                                    <p><b>Category</b>: {{ $catrow->category_name }}</p>
                                </div>
                        ...
                        <div class="col-sm-12">
                            <div class="vendor-price">
                                <a href="{{ url("buyer/$row->company_slug/$row->unique_code") }}" class="btn btn-primary btn-sm">Read More</a>&nbsp;&nbsp;
                                <a href="{{ url("buyer/$row->company_slug/$row->unique_code") }}" class="btn btn-primary btn-sm">Send Mail</a>
                            </div>
                        </div>
